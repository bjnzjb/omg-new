...
import { callApiStatus, defaultDateFormat } from "app/constants";
...
function UpdateProjectDialog(props) {
  const { open, setOpen, handleUpdateProjectSuccess, projectBaseInfo } = props;

  const [updateProjectStatus, setUpdateProjectStatus] = useState(callApiStatus.idle);
  ...
  const initialValues = useMemo(
    () => ({
      ...
      key: projectBaseInfo.key,
      customer_id: projectBaseInfo.customer_id,
      project_type_id: projectBaseInfo.project_type_id,
      billable: projectBaseInfo.billable,
      start_date: moment(projectBaseInfo.start_date).format(defaultDateFormat),
      end_date: moment(projectBaseInfo.end_date).format(defaultDateFormat),
      introduction: projectBaseInfo.introduction,

      // Person In Charge
      unit_id: projectBaseInfo.unit_id,
      pm_id: projectBaseInfo.pm_id,
      sm_id: projectBaseInfo.sm_id,
      sale_id: projectBaseInfo.sale_id,
    }),
    [projectBaseInfo],
  );
  ...
  const handleResetAndClose = () => {
    ...
    setUpdateProjectStatus(callApiStatus.idle);
  };

  const handleUpdateProject = (formData) => {
    if (updateProjectStatus === callApiStatus.loading) return;
    const projectInfo = {
      name: formData.name,
      key: formData.key,
      code: genProjectCode,
    ...
    setUpdateProjectStatus(callApiStatus.loading);
    projectServices
      .updateProjectInfo(projectBaseInfo.id, projectInfo)
      .then((res) => {
    ...
    <CommonDialog open={open} handleClose={handleClose}>
      ...
      >
        <div className="w-full h-auto flex flex-col items-center justify-center">
          <h2 className="w-full h-12 text-[32px] font-medium text-start">Update new project</h2>
      ...
      <PageLoadingBackdrop open={updateProjectStatus === callApiStatus.loading} />
    </CommonDialog>
  );
}
