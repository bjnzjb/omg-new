...
function UpdateResourceAllocation(props) {
  const { open, closeDialog, projectId, handleUpdateProjectSuccess, updateResourceInfo, handleShowConfirmDelete } =
    props;

  const { memberOptions, loadingMemberStatus, fetchMemberOptions } = useFetchMemberOptions();

  const [updateResourceStatus, setUpdateResourceStatus] = useState(callApiStatus.idle);
  ...
  const initialValues = useMemo(
    () =>
      updateResourceInfo?.id
        ? {
            member_id: updateResourceInfo.member_id,
            role_id: updateResourceInfo.role_id,
            start_date: moment(updateResourceInfo.start_date).utc().format(defaultDateFormat),
            end_date: moment(updateResourceInfo.end_date).utc().format(defaultDateFormat),
            effort: updateResourceInfo.effort,
            memo: updateResourceInfo.memo,
          }
        : { member_id: undefined, role_id: undefined, start_date: "", end_date: "", effort: 1, memo: "" },
    [updateResourceInfo],
  );
  ...
  const handleResetAndClose = () => {
    ...
    setUpdateResourceStatus(callApiStatus.idle);
  };

  const handleUpdateResource = (formData) => {
    console.log("formData", formData);
    if (updateResourceStatus === callApiStatus.loading) return;
    const resourceInfo = {
    ...
    setUpdateResourceStatus(callApiStatus.loading);
    projectServices
      .updateProjectResource(updateResourceInfo.id, resourceInfo)
      .then((res) => {
        toast.success("Update resource success.");
        handleResetAndClose();
        const newProjectData = res.data;
        if (typeof handleUpdateProjectSuccess === "function") handleUpdateProjectSuccess(newProjectData);
      })
      .catch((error) => {
    ...
    <CommonDialog open={open} handleClose={handleClose}>
      ...
      >
        <div className="w-full h-auto flex flex-col items-center justify-center">
          <h2 className="w-full h-12 text-[32px] font-medium text-start">Update resource</h2>
        </div>
      </DialogTitle>
      ...
      >
        <div className="w-full h-auto flex items-center justify-between">
          <button
            ...
            className="text-base px-4 py-2 rounded bg-red-400 hover:bg-red-500 text-white"
          >
            Delete
          </button>
      ...
      <ConfirmClose
        ...
        title="Confirm close update resource!"
      >
        <p className="py-4">There are some changes, are you sure you want to skip it?</p>
      </ConfirmClose>

      <PageLoadingBackdrop open={updateResourceStatus === callApiStatus.loading} />
    </CommonDialog>
  );
}
