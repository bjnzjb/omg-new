...
function UpdateCustomerDialog(props) {
  const { open, setOpen, updateCustomerInfo, handleUpdateCustomerSuccess } = props;

  const [updateCustomerStatus, setUpdateCustomerStatus] = useState(callApiStatus.idle);
  ...
  const { handleSubmit, errors, values, setFieldValue, resetForm, touched, handleBlur, setFieldTouched } = useFormik({
    initialValues: {
      ...
      key: updateCustomerInfo.key || "",
      market: updateCustomerInfo.market || "",
      language: updateCustomerInfo.language || "",
      sales: updateCustomerInfo.sales || "",
      customer_email: updateCustomerInfo.customer_email || "",
      customer_phone: updateCustomerInfo.customer_phone || "",
      customer_address: updateCustomerInfo.customer_address || "",
      customer_introduction: updateCustomerInfo.customer_introduction || "",

      // Representativ Person
      representative_customer_name: updateCustomerInfo.representative_customer_name || "",
      representative_customer_email: updateCustomerInfo.representative_customer_email || "",
      representative_customer_phone_number: updateCustomerInfo.representative_customer_phone_number || "",
      // Avatar
      customerImage: "",
    },
    onSubmit,
  ...
  const checkHasChanged = () => {
    ...
    console.log("updateCustomerInfo", updateCustomerInfo);

    if (
      values.name !== updateCustomerInfo.name ||
      values.key !== updateCustomerInfo.key ||
      values.market !== updateCustomerInfo.market ||
      values.language !== updateCustomerInfo.language ||
      values.sales !== updateCustomerInfo.sales ||
      values.customer_email !== updateCustomerInfo.customer_email ||
      values.customer_phone !== updateCustomerInfo.customer_phone ||
      values.customer_address !== updateCustomerInfo.customer_address ||
      values.customer_introduction !== updateCustomerInfo.customer_introduction ||
      values.representative_customer_name !== updateCustomerInfo.representative_customer_name ||
      values.representative_customer_email !== updateCustomerInfo.representative_customer_email ||
      values.representative_customer_phone_number !== updateCustomerInfo.representative_customer_phone_number
    )
      return true;
    return false;
  };
  ...
  const handleResetAndClose = () => {
    ...
    setUpdateCustomerStatus(callApiStatus.idle);
  };

  const handleUpdateCustomer = (formData) => {
    if (updateCustomerStatus === callApiStatus.loading) return;
    ...
    setUpdateCustomerStatus(callApiStatus.loading);
    customerServices
      .updateCustomer(updateCustomerInfo.id, customerInfo)
      .then((res) => {
        toast.success("Update customer success.");
        handleResetAndClose();
        if (typeof handleUpdateCustomerSuccess === "function") handleUpdateCustomerSuccess(res.data);
      })
      .catch((error) => {
        setUpdateCustomerStatus(callApiStatus.loading);
    ...
    <CommonDialog open={open} handleClose={handleClose}>
      ...
      >
        <div className="w-full h-auto flex flex-col items-center justify-center">
          <h2 className="w-full h-12 text-[32px] font-medium text-start">Update customer</h2>
      ...
      <PageLoadingBackdrop open={updateCustomerStatus === callApiStatus.loading} />
    </CommonDialog>
  );
}
